...
export type OrderResponseDto = {
  ...
  sellerFee?: SellerFeeDto;
  /**
   * The viewing organization's own role on this order. Anonymity-safe (says nothing about the counterparty); lets the client show seller-vs-buyer copy without inferring role from a fee's presence.
   */
...
export type InvoiceEventResponseDto = {
  /**
   * When it happened (ISO 8601)
   */
  at: string;
  /**
   * User who acted; absent for system and customer actions
   */
  actor?: string;
  action:
    | "created"
...
export type InvoiceResponseDto = {
  ...
  /**
   * True when the invoice is pending and past its due date. A signal, not a status: an overdue invoice is still payable.
   */
  isOverdue: boolean;
  items: Array<InvoiceItemResponseDto>;
  taxType: "fixed" | "percentage";
  /**
   * Minor units if FIXED, percentage if PERCENTAGE
   */
  taxValue: number;
  discountType: "fixed" | "percentage";
  /**
  ...
  notes?: string;
  /**
   * The customer-facing pay page URL, once sent
   */
  ...
  cancelledAt?: string;
  /**
   * What the settling deposit actually carried, custody minor units
   */
  settledAmount?: number;
  /**
   * settledAmount minus the invoice total: negative = underpaid, positive = overpaid, 0 = exact
   */
  settlementVariance?: number;
  /**
   * Append-only audit trail
   */
  events: Array<InvoiceEventResponseDto>;
  /**
   * ISO 8601
   */
  createdAt: string;
...
export type InvoiceTabCountsDto = {
  all: number;
  draft: number;
  pending: number;
  paid: number;
  cancelled: number;
  /**
   * Pending invoices past their due date
   */
  overdue: number;
};

export type InvoicePageResponseDto = {
  ...
  limit: number;
  offset: number;
  tabCounts: InvoiceTabCountsDto;
};

export type InvoiceCollectableCurrenciesResponseDto = {
  /**
   * Currencies the org can collect invoice payments in right now; anything else would fail at send
   */
...
export type ResendInvoiceDto = {
  /**
   * Corrected customer email; replaces the stored address before resending (the recovery for a typo or a bounce)
   */
  customerEmail?: string;
};
...
export type PublicInvoiceResponseDto = {
  ...
  /**
   * The still-valid payment destination already minted for this invoice, when one exists — so a returning customer resumes with the same instructions instead of minting a fresh destination
   */
  instructions?: WalletFundDetailsResponseDto;
};
...
export type CompleteReturnInspectionDto = {
  /**
   * Inspector notes on the condition of the returned asset. Shown on the completed stage.
   */
  notes?: string;
};
...
export type PersonScreeningSubjectDto = {
   ...
   */
  bvnCheck?: PersonBvnCheckDto | null;
};
...
export type AdminProviderBindingDto = {
  ...
  stalledAt?: string;
  /**
   * The failure that was repeating when retries stopped
   */
  stallReason?: string;
  /**
   * How many attempts in a row produced the same outcome and error
   */
  consecutiveSameOutcome?: number;
  /**
   * Why the binding is not active, bucketed by whose move it is: transient (retry helps), data_actionable (org must correct data via request-changes), provider_blocked (waiting on the provider; retrying is futile), stalled (retries stopped after repeated identical failures; fix the cause then retry). Null when healthy or normally in progress.
   */
  blockedBy: "transient" | "data_actionable" | "provider_blocked" | "stalled";
};
...
export type InvoicesControllerFindAllData = {
  ...
  query?: {
    ...
    offset?: number;
    /**
     * Status facet; CSV or repeated param
     */
    status?: Array<"draft" | "pending" | "paid" | "cancelled">;
    /**
     * True narrows to pending invoices past their due date (the Overdue tab); overrides the status facet
     */
    overdue?: boolean;
    /**
     * Search by customer name or title (literal match)
     */
    search?: string;
  };
  url: "/api/v1/invoices";
...
export type InvoicesControllerCollectableCurrenciesData = {
  body?: never;
  path?: never;
  query?: never;
...
export type InvoicesControllerCollectableCurrenciesResponses = {
  200: InvoiceCollectableCurrenciesResponseDto;
};

export type InvoicesControllerCollectableCurrenciesResponse =
  InvoicesControllerCollectableCurrenciesResponses[keyof InvoicesControllerCollectableCurrenciesResponses];
...
export type InvoicesControllerDeleteResponse =
  InvoicesControllerDeleteResponses[keyof InvoicesControllerDeleteResponses];
...
export type InvoicesControllerResendData = {
  body: ResendInvoiceDto;
  path: {
    id: string;
  };
  query?: never;
...
export type InvoicesControllerResendResponses = {
  200: InvoiceResponseDto;
};

export type InvoicesControllerResendResponse =
  InvoicesControllerResendResponses[keyof InvoicesControllerResendResponses];

export type InvoicesControllerCancelData = {
  body?: never;
  path: {
    id: string;
  };
...
export type InvoicesControllerCancelResponses = {
  200: InvoiceResponseDto;
};

export type InvoicesControllerCancelResponse =
  InvoicesControllerCancelResponses[keyof InvoicesControllerCancelResponses];
...
export type InvoicesPayControllerProvisionInstructionsResponses = {
  201: WalletFundDetailsResponseDto;
};
...
export type AdminTransactionsControllerCompleteReturnInspectionData = {
  body: CompleteReturnInspectionDto;
  path: {
    /**
     * Transaction ID
     */
...
export type AdminTransactionsControllerCompleteReturnInspectionResponses = {
  /**
   * Updated transaction with the return inspection completed.
   */
  200: TransactionResponseDto;
};

export type AdminTransactionsControllerCompleteReturnInspectionResponse =
  AdminTransactionsControllerCompleteReturnInspectionResponses[keyof AdminTransactionsControllerCompleteReturnInspectionResponses];
...
export type AdminOrganizationsControllerFindAllData = {
  ...
  query?: {
    ...
    approved?: string;
    /**
     * Owner lifecycle stage: 'active' (approved), 'onboarding' (registered, never submitted for review), 'in_review' (submitted / pending review / action required). Omit for all stages.
     */
    lifecycle?: "active" | "onboarding" | "in_review";
    /**
...
export type QqCatalogControllerMatchTemplatesResponse =
  QqCatalogControllerMatchTemplatesResponses[keyof QqCatalogControllerMatchTemplatesResponses];
